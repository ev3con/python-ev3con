...
@create_ev3_property(
	...
	driver_name={'read_only': True, 'property_type': Ev3IntType},
    ...
    #~ hold_pid/Kd={'read_only': False, 'property_type': Ev3IntType}, FUNKTIONIERT LEIDER NICHT SO
    #~ hold_pid/Ki={'read_only': False, 'property_type': Ev3IntType},
    #~ hold_pid/Kp={'read_only': False, 'property_type': Ev3IntType},
    ...
    #~ speed_pid/Kd={'read_only': False, 'property_type': Ev3IntType}, EBENFALLS NICHT
    #~ speed_pid/Ki={'read_only': False, 'property_type': Ev3IntType},
    #~ speed_pid/Kp={'read_only': False, 'property_type': Ev3IntType},
...
class Motor(Ev3Dev):
	...
	def get_commands(self):#Zeigt die moeglichen Befehle an
		print(self.commands)
		print(self.stop_commands)
	
	def	set_command(self,com):#Besser als die Attribute command uns stop_command selber zu setzen
		if com in self.commands:
			self.command=com
		elif com in self.stop_commands:
			self.stop_command=com
		else :
			print('Befehl nicht vorhanden')
	...
	def run_position(self, position_sp, speed_sp, relative=False, **kwargs):
		...
		if relative : 
			...
			#~ self.start() 
	def set_hold_pid(self,ki,kp,kd):
		self.write_value('hold_pid/Kd', kd) 
		self.write_value('hold_pid/Kp', ki) 
		self.write_value('hold_pid/Ki', kp) 
	def set_speed_pid(self,ki,kp,kd):
		self.write_value('speed_pid/Kd', kd) 
		self.write_value('speed_pid/Kp', kp) 
		self.write_value('speed_pid/Ki', ki) 	
